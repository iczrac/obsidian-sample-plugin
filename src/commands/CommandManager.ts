...
扩展列功能允许在八字表格中自动添加大运、流年、流月、流日、流时等时间层级的分析列。
...
| 参数 | 完整写法 | 简洁写法 | 数字写法 | 中文写法 | 说明 |
|------|----------|----------|----------|----------|------|
| 不扩展 | \`extend: none\` | \`ex: no\` | \`e: 0\` | \`extend: 无\` | 只显示四柱 |
| 当前时间 | \`extend: auto_current\` | \`ex: cur\` | \`e: 1\` | \`extend: 当前\` | 扩展到当前流时 |
| 流日 | \`extend: auto_day\` | \`ex: day\` | \`e: 2\` | \`extend: 流日\` | 扩展到流日层级 |
| 流月 | \`extend: auto_month\` | \`ex: month\` | \`e: 3\` | \`extend: 流月\` | 扩展到流月层级 |
| 特殊宫位 | \`extend: special_palaces\` | \`ex: palace\` | \`e: 4\` | \`extend: 宫位\` | 胎元命宫身宫 |
| 自定义时间 | \`extend: custom\` | \`ex: custom\` | \`e: 5\` | \`extend: 自定义\` | 扩展到指定时间 |

### 扩展列使用技巧

1. **快速查看当前运势**：使用 \`e: 1\` 是最常用的功能
2. **分析宫位信息**：使用 \`e: 4\` 查看胎元、命宫、身宫
3. **自定义时间分析**：使用 \`e: 5\` 配合 \`t: 时间\` 查看特定时间
4. **参数优先级**：完整参数 > 简洁参数 > 数字参数
5. **混合使用**：可以在同一个代码块中混合使用不同类型的参数
...
| 参数 | 说明 | 示例 |
|------|------|------|
| \`extend / ex / e\` | 扩展类型 | \`e: 1\` (当前时间) / \`e: 4\` (宫位) / \`e: 0\` (不扩展) |
| \`extendTarget / to / t\` | 自定义目标时间 | \`t: 2025-01-01 12:00\` |
...
5. **扩展列使用**：
   - 日常分析使用 \`e: 1\` 查看当前运势
   - 学习宫位使用 \`e: 4\` 分析胎元命宫身宫
   - 特定时间分析使用 \`e: 5\` 配合 \`t\` 参数
   - 简洁输入使用数字参数，详细分析使用完整参数
...
3. 🚀 **体验扩展列功能** - 尝试不同的 \`e\` 参数值（0-5）
4. 🔗 **创建双链笔记** - 为感兴趣的人物创建专门的笔记
5. ⚙️ **调整插件设置** - 根据个人喜好定制功能
6. 📚 **建立学习体系** - 创建自己的八字学习笔记库

### 🚀 扩展列功能快速体验

试试这些快捷操作：
- 将任意八字的 \`style: 3\` 改为 \`style: 3\` 并添加 \`e: 1\`
- 体验 \`e: 4\` 查看胎元、命宫、身宫
- 使用 \`e: 5\` 和 \`t: 2025-01-01 12:00\` 查看未来运势
